...
4. Added a visible app version + last-updated timestamp, same convention as URTO: an `APP_VERSION` constant at the top of `app.py`, rendered in a footer at the bottom of the page. **Bump it by hand on every future shipped change** — current: v1.4.6. (The timestamp next to it used to be a hand-typed `APP_VERSION_DATE` constant too, but that's now computed automatically — see item 15.)
...
16. Reworked Fit Maker from a two-column-plus-collapse layout into a stable three-way split: Tops on the left, Bottoms on the right, and a **live preview** in the middle (`#fitPreviewStage`) that updates instantly on every tap using the same stacked-layers renderer (`ovLayer`/`tightenAll`) the outfit lightbox already used — no more clicking a separate "Preview Outfit" button to see it (that button/modal-trigger is gone from this screen; `previewFit()` was removed as dead code). Also fixed `toggleFit()` allowing more than one top or one bottom to be selected at once — it silently just added every tapped item to the set with no exclusivity — now picking a new top/bottom deselects whichever piece of that same type was already picked, so it's always at most one top + one bottom, matching what the screen's copy always said ("tap a top, then a bottom").
