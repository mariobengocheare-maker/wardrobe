...
4. Added a visible app version + last-updated timestamp, same convention as URTO: `APP_VERSION`/`APP_VERSION_DATE` constants at the top of `app.py`, rendered in a footer at the bottom of the page. **Bump both by hand on every future shipped change**, timestamp in **Eastern time** (mind DST — EDT in summer, EST in winter) — current: v1.4.4, updated Jul 30, 2026 9:35 PM EDT.
